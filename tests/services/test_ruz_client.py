...
async def test_fetch_week_returns_flat_list_of_lessons_with_day_date():
    ...
    with aioresponses() as m:
        m.get("https://ruz.example/api/v1/ruz/scheduler/43328?date=2026-05-25", payload=FIXTURE)
        lessons = await client.fetch_week(43328, date(2026, 5, 25))
...
async def test_fetch_week_raises_on_500():
    ...
    with aioresponses() as m:
        ...
        m.get("https://ruz.example/api/v1/ruz/scheduler/43328?date=2026-05-25", status=500)
        m.get("https://ruz.example/api/v1/ruz/scheduler/43328?date=2026-05-25", status=500)
        with pytest.raises(RuzError):
            await client.fetch_week(43328, date(2026, 5, 25))
...
def test_public_url_format():
    client = RuzClient(base_url="https://ruz.example", faculty_id=125, timeout=5)
    url = client.public_url(43328, date(2026, 5, 25))
    # Без zero-padding в дате, как в реальном RUZ
    assert url == "https://ruz.example/faculty/125/groups/43328?date=2026-5-25"
