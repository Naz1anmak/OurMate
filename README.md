...
**Telegram‑бот с интеграцией LLM, расписанием пар и планировщиком поздравлений**
...
- Парсить `calendar*.ics`, кешировать расписание и отдавать пары на сегодня/завтра командами или рассылкой в 08:00 (если пары есть)
...
- Команды «пары» / «пары завтра» (в группе, при упоминании бота или ответе)
- Ежедневная рассылка пар на сегодня в 08:00, только если пары есть
...
PROMPT_TEMPLATE_BIRTHDAY_ACTIVE='Поздравь активных студентов {mentions}…'   # промпт для активных
...
Если `interacted_with_bot = false`, бот **не будет поздравлять** этого пользователя в беседе. Владелец получит уведомление в ЛС о неактивных именинниках.
...
| `проверка ссылок` | ЛС/Группа | Диагностика ссылок и активации пользователей |
| `др` | ЛС/Группа | Ближайший день рождения |
| `др 123456789` или `др @username` | ЛС/Группа | Дата дня рождения пользователя по id или username |
| `пары` | ЛС/Группа | Пары на сегодня (в группе — при упоминании бота или ответе) |
| `пары завтра` | ЛС/Группа | Пары на завтра (в группе — при упоминании бота или ответе) |
...
- `др 123456789` или `др @username` — показать дату дня рождения по user_id или username
- `пары` — пары на сегодня
- `пары завтра` — пары на завтра
- `отписаться` — отключить поздравления (**только в ЛС с ботом**)
- `help` / `команды` — справка по командам бота

* Нужно упомянуть бота перед командой с помощью @ или ответить на его сообщение

**Примечания:**
- Чтобы **получать поздравления**, напишите боту любое сообщение в ЛС (активирует поле `interacted_with_bot`)
