...
- Парсит `.ics` при старте и кеширует в `data/cache/schedule_cache.json`. Поддерживает два режима: single-group (все файлы лежат прямо в `data/`, шаблон `calendar*.ics`) и multi-group (по подпапке на группу: `data/<code>/calendar*.ics`)
...
**Single-group режим.** Сохрани в `data/` файлы `calendar.ics`, `calendar-2.ics` и т.п. (шаблон `calendar*.ics`). При старте парсятся все файлы и кешируются в `data/cache/schedule_cache.json`.

**Multi-group режим.** Если в чате учатся несколько групп с частично разным расписанием, разложи `.ics` по подпапкам — по одной на группу:
...
- Имя подпапки = код группы. Имена файлов внутри произвольные, лишь бы матчился `calendar*.ics`.
- Бот сам определяет режим: подпапки с `calendar*.ics` есть — multi-group, нет — single-group fallback по `SCHEDULE_FILES_PATTERN`.
- Отображаемое имя группы = `SCHEDULE_GROUP_NAME_PREFIX` + код. Пример: при `SCHEDULE_GROUP_NAME_PREFIX=з5130903/` папка `40001` отображается как `з5130903/40001`.
- Совпадающие пары двух групп показываются одним блоком; различающиеся — двумя блоками с заголовком `❗️ ... для <имя группы>`.

**Где взять `.ics`.** Для СПбПУ удобно RUZ — на странице расписания группы есть кнопка iCal. Прямая ссылка:

```
https://ruz.spbstu.ru/faculty/<faculty_id>/groups/<group_id>/ical?date=YYYY-MM-DD
```

Скачанный файл клади в подпапку соответствующей группы. Автоматическая загрузка пока не реализована — отдельный тикет.
