...
- Автоматически тянуть расписание из RUZ (JSON-API), нормализовать тип занятия (Лекция/Практика/Лаб./Экзамен/Зачёт) и отдавать пары на сегодня/завтра командами или рассылкой
...
### 3. **Расписание пар (RUZ JSON-API)**
- Тянет расписание из публичного JSON-API RUZ (`/api/v1/ruz/scheduler/<group_id>?date=YYYY-MM-DD`), сохраняет в `data/<code>/schedule.json`. Поддерживает несколько групп — по подпапке на каждую
...
- Diff: после обновления, если расписание изменилось — бот шлёт в чат сообщение со списком ➕ добавленных / ➖ удалённых / ✏️ изменённых пар по датам (с подписью «для <code>» когда групп >1)
...
- Команды «пары» / «пары завтра» (в группе, при упоминании бота или ответе; в ЛС — для владельца и пользователей из списка дней рождения); перед ответом — lazy-refresh с TTL (по умолчанию 60 мин), чтобы не дёргать RUZ на каждый запрос
- «обнови расписание» — ручное принудительное обновление: показывает diff либо «расписание не изменилось», при недоступности RUZ — гиперссылку на frontend и время прошлого снимка
...
│   │   └── schedule.json                    # Снимок расписания из RUZ JSON-API
│   └── cache/                               # Кеш
...
### 5. **Расписание (RUZ JSON-API)**

Бот сам тянет расписание из RUZ — никаких ручных `.ics` не нужно. Подключение групп:

1. В `.env` указать `RUZ_BASE_URL` (домен RUZ-портала твоего вуза) и `RUZ_FACULTY_ID` (числовой ID факультета из URL).
2. Для каждой группы добавить `RUZ_GROUP_<CODE>=<RUZ group_id>`, где `<CODE>` — твой код подпапки (любой удобный, латиница/цифры), а `<RUZ group_id>` — числовой ID из URL страницы группы.
...
RUZ_BASE_URL=https://<твой-ruz-домен>
RUZ_FACULTY_ID=125
...
- В командах «пары» / «пары завтра» — `ensure_fresh` с TTL: если последний снимок свежее `RUZ_LAZY_TTL_MIN`, не дёргаем RUZ.
- По команде «обнови расписание» — принудительный `force_refresh` + обновление закрепа.

**Что в diff.** Сообщение «🗓️ Расписание обновилось» содержит блоки по датам:
- ➕ — добавленные пары (время, предмет, тип);
- ➖ — удалённые;
- ✏️ — изменения времени (то же название, новый интервал).
...
**Отображаемое имя группы.** `SCHEDULE_GROUP_NAME_PREFIX` + код подпапки. Пример: при `SCHEDULE_GROUP_NAME_PREFIX=з51904/` подпапка `40001` отображается как `з51904/40001`. Совпадающие пары двух групп показываются одним блоком; различающиеся — двумя блоками с заголовком `❗️ ... для <имя группы>`.
...
**Сеть недоступна?** Если все недели для группы не загрузились — старый снимок остаётся (`fetched_at` не сдвигается). На команде «обнови расписание» бот ответит «⚠️ Расписание недоступно, остался прошлый снимок (от DD.MM HH:MM)» с гиперссылкой на frontend RUZ. Утренняя рассылка и закреп продолжат работать с тем что есть.
