...
- Стримит ответ по мере генерации: в беседе — редактирует плейсхолдер, в ЛС — рисует черновик (`SendMessageDraft`), без отдельного сообщения-заглушки
- Троттлинг стрима: пороги `1.0s / 100 символов` в беседе и `0.5s / 40` в ЛС, плюс скользящий бюджет в 14 эдитов в минуту на чат — чтобы финальный эдит всегда проходил мимо flood control; при `retry_after` ждёт и догружает
...
- Когда расписание меняется — шлёт в чат diff по датам (новые / заменившие слот / удалённые / перенесённые / изменённые пары)
- Команды «пары» / «пары завтра» (в беседе — при упоминании бота или ответе; в ЛС — владельцу и пользователям из списка дней рождения); если на выбранный день пар нет, бот показывает ближайшие будущие
- «обнови расписание» — ручное принудительное обновление: diff либо «расписание не изменилось»
- Ежедневная рассылка пар на сегодня в `SCHEDULE_SEND_HOUR:MM` (по умолчанию 08:00), только если пары есть — отключается флагом

Подробности — расписание обновлений, формат diff, поведение при недоступности API — в разделе [«Расписание»](#6-расписание-автообновление-через-json-api) ниже.

### 4. **Расписание в закрепе**
- Бот держит закреплённое сообщение со списком следующих `PINNED_SCHEDULE_DAYS_AHEAD` учебных дней (по умолчанию 7)
- Обновление ежедневное, плюс сразу после старта бота — не дожидаясь планового времени
- Если ближайших пар нет — закреп удаляется автоматически
- Утренняя рассылка включается/выключается флагом: можно жить либо на закрепе, либо на рассылке
...
На нетехнические обращения — болтовню, шутки, подколы — бот может ответить **голосовым
сообщением** клоном голоса (MiniMax TTS). Решает сама LLM: такой ответ она помечает служебным
маркером `[voice]`. Технические вопросы, расписание, списки, напоминания — всегда текстом.

- Голос возможен в беседе и в ЛС (в ЛС — только владельцу и пользователям из `birthdays.json`).
- Ответ короче `VOICE_MIN_CHARS` (по умолчанию 35 символов) или с вызовом тула — текстом.
- Изредка модель может вставить в реплику паузу вида `<#0.5#>` — перед отправкой текстом (если
  синтез не удался) паузы вырезаются, в голосе они звучат естественно.
- Пока идёт синтез, в шапке чата висит «записывает голосовое».
- Любой сбой синтеза — тот же ответ уходит текстом, владельцу приходит уведомление.
- MiniMax отдаёт mp3, `ffmpeg` в образе перекодирует его в OGG/Opus — так Telegram показывает
  именно голосовое с дорожкой громкости.

**Настройка:** `MINIMAX_API_KEY` и `MINIMAX_VOICE_ID` в `.env`; пустое значение = фича выключена.

**Как получить `voice_id`:**
1. Создать клон голоса в веб-интерфейсе MiniMax (раздел Voice Clone) — там же можно прослушать
   результат перед тем, как подключать к боту.
2. Узнать `voice_id` клона через API:
   `curl -X POST https://api.minimax.io/v1/get_voice -H "Authorization: Bearer $MINIMAX_API_KEY" -H "Content-Type: application/json" -d '{"voice_type":"voice_cloning"}'`
3. Записать полученный `voice_id` в `.env` как `MINIMAX_VOICE_ID`.

Клон можно создать и напрямую через API (без веб-интерфейса) — загрузить образец голоса
(`/v1/files/upload`, `purpose=voice_clone`) и вызвать `/v1/voice_clone` со своим `voice_id`; см.
[документацию MiniMax](https://platform.minimax.io/docs/guides/speech-voice-clone).

> ⚠️ Клон удаляется, если за **7 дней** им ни разу не синтезировали речь через API. Первая
> озвучка клона через API списывает **$1.50**. Баланс для API пополняется отдельно от кредитов
> веб-приложения — проверьте, что на API-счету есть средства, иначе синтез будет падать ошибкой
> авторизации.

### 10. **Дневные лимиты обращений к LLM**

Чтобы один человек не сжёг весь бюджет LLM за вечер, у каждого свой суточный счётчик — отдельный
на ЛС и на беседу. Лимит исчерпан — бот отвечает короткой заглушкой (варианты текстов в
`usage_limit_variants.py`), LLM при этом не дёргается; детерминированные команды (`пары`, `др`,
`напоминания`, `списки`) работают как обычно. Счётчики живут в SQLite (`data/usage.db`), сутки
считаются по `TIMEZONE`.

| Переменная | По умолчанию | Описание |
|---|---|---|
| `PM_DAILY_MSG_CAP` | `30` | Лимит обращений к LLM в ЛС (на пользователя в сутки) |
| `CHAT_DAILY_MSG_CAP` | `30` | Лимит обращений к LLM в беседе (на пользователя в сутки) |
| `USAGE_DB_PATH` | `data/usage.db` | Путь к SQLite-базе счётчиков |
| `USAGE_RETENTION_DAYS` | `30` | Срок хранения строк счётчиков; чистка при старте |

### 11. **Приветствие новых участников беседы**

Новый участник в основной беседе — бот здоровается (`src/bot/services/welcome.py`, апдейты
`chat_member`). Если рядом лежит файл из `WELCOME_GIF_FILE`, приветствие уходит гифкой с подписью,
иначе просто текстом.

| Переменная | По умолчанию | Описание |
|---|---|---|
| `WELCOME_ENABLED` | `true` | Приветствовать новых участников |
| `WELCOME_GIF_FILE` | `data/welcome.mp4` | Гифка приветствия (mp4 без звука или `.gif`); нет файла — только текст |

### 12. **Гибкая конфигурация через `.env`**
...
- **Логирование** — единый формат `%(asctime)s [%(levelname)s] %(name)s: %(message)s` для всех строк (наш код + aiogram + httpx). Уровень настраивается переменной `LOG_LEVEL` (по умолчанию `INFO`).
...
- **Доступ** — единый источник правды `src/bot/handlers/access.py`: командное слово → аудитория → решение. Кто что может — в таблице [«Команды»](#-команды). Команды владельца гейтятся по `OWNER_CHAT_ID` и читают только локальные файлы (лог, `birthdays.json`), системные утилиты не дёргают
- **Лимиты длины** — LLM-ответ режется по видимой длине (`_trim_html`: 4050 символов при жёстком потолке 4700 с учётом HTML-тегов), выдача логов — по 4000 (лимит сообщения Telegram)
...
| `напоминания` | 🌍 | Список активных напоминаний (в беседе — её общие, в ЛС — свои личные) |
...
* В ЛС команды, кроме `help`/`команды` и `напоминания`, доступны только владельцу и пользователям из `data/birthdays.json`.
...
│   │   │   ├── chat_commands.py             # Публичные команды (др/пары/обнови расписание/пинг/списки)
│   │   │   ├── owner_commands.py            # Команды владельца (logs/проверка ссылок)
│   │   │   ├── chat_member.py               # Вход/выход участников: приветствие и чистка списков
│   │   │   ├── llm_flow.py                  # Стриминг LLM-ответов + рендер (тул-флоу, голос)
│   │   │   ├── *_callbacks.py               # Кнопки: notes / ping / reminder
│   │   │   ├── notes_reply.py               # Реплай на карточку списка (имя, примечание)
...
│   │   │   ├── notes_*.py                   # Списки/очереди: тулы, бизнес-логика, SQLite
│   │   │   ├── ping_service.py / ping_store.py          # Пинг-лист и его SQLite-хранилище
│   │   │   ├── usage_limit.py / usage_limit_store.py    # Дневные лимиты обращений к LLM
│   │   │   ├── welcome.py                   # Приветствие новых участников беседы
│   │   │   ├── tts_service.py               # Синтез голоса: MiniMax t2a_v2 (mp3) → ffmpeg → OGG/Opus
...
├── tests/                                   # Pytest: хендлеры, сервисы, планировщики
├── data/                                    # Данные приложения (не в git)
│   ├── birthdays.json                       # Дни рождения
│   ├── reminders.db / ping.db / notes.db    # SQLite: напоминания, пинг-лист, списки
│   ├── usage.db                             # SQLite: дневные счётчики обращений к LLM
│   ├── logs/bot.log                         # Логи с ротацией (10 МБ × 5)
...
├── docker-compose.override.yml.example      # Dev-маунт src/ и tests/ в контейнер (см. make override)
├── Makefile                                 # Цели для разработки и эксплуатации
├── pyproject.toml                           # Конфиг pytest
├── requirements.txt / requirements-dev.txt  # Зависимости Python (прод / тесты)
...
| `make override` | создать `docker-compose.override.yml` из примера (dev-маунт `src/` и `tests/`) |
| `make test` | прогон тестов в контейнере на текущем коде |
| `make test-cov` | то же с отчётом о покрытии |

### 4. **Тесты**

```bash
make test        # 481 тест, ~2 секунды
```

Цель сама вызывает `make override`: без `docker-compose.override.yml` контейнер видит код,
**запечённый в образ** при сборке, и свежие правки в тестах не участвуют. Тесты гоняются только
внутри контейнера — в системном Python зависимостей проекта нет.


### 5. **Пример `data/birthdays.json`**
...
### 6. **Расписание (автообновление через JSON-API)**
...
Если у нескольких групп на одну дату полностью совпадает состояние до и после, блок объединяется в один — и при полном покрытии всех известных групп суффикс «для …» опускается; иначе в заголовке появляется «для GRP_A и GRP_B» (через `_format_groups`).
...
Если в `make tail` всплыло `[WARNING] root: Не удалось открыть data/logs/bot.log для логов: [Errno 13] Permission denied: 'data/logs'` — это та самая история с правами на `data/` из раздела «Развертывание на сервере».
