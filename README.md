...
- Автоматически тянуть расписание из RUZ СПбПУ (JSON-API), нормализовать тип занятия (Лекция/Практика/Лаб./Экзамен/Зачёт) и отдавать пары на сегодня/завтра командами или рассылкой
...
- Тянет расписание из публичного JSON-API RUZ СПбПУ (`/api/v1/ruz/scheduler/<group_id>?date=YYYY-MM-DD`), сохраняет в `data/<code>/schedule.json`. Поддерживает несколько групп — по подпапке на каждую
...
Бот сам тянет расписание из RUZ СПбПУ — никаких ручных `.ics` не нужно. Подключение групп:

1. В `.env` указать `RUZ_FACULTY_ID` (например, `125` — Институт компьютерных наук).
2. Для каждой группы добавить `RUZ_GROUP_<CODE>=<RUZ group_id>`, где `<CODE>` — твой код подпапки (любой удобный, латиница/цифры), а `<RUZ group_id>` — числовой ID из URL страницы группы на `ruz.spbstu.ru`.
...
RUZ_BASE_URL=https://ruz.spbstu.ru
RUZ_FACULTY_ID=125
