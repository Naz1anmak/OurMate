...
GET_SCHEDULE_SCHEMA = {
    ...
    "function": {
        ...
        "description": (
            ...
            "пары по RUZ (включая чужие параллели потока); teachers — преподаватели. "
            "ВАЖНО: lesson_groups и teachers — справочный контекст, НЕ выводи их в обычный ответ. "
            "По умолчанию показывай только время, тип и предмет. Преподавателя называй ТОЛЬКО на "
            "прямой вопрос «кто ведёт», состав параллелей — ТОЛЬКО на «с кем у нас занятие» (бери "
            "lesson_groups за вычетом наших групп). Не упоминай эти поля по своей инициативе."
        ),
        "parameters": {
            "type": "object",
            "properties": {
...
FIND_CLASSES_BY_SUBJECT_SCHEMA = {
    ...
    "function": {
        ...
        "description": (
            ...
            "Также у события есть lesson_groups (весь состав пары по RUZ) и teachers (преподаватели) — "
            "это справочный контекст для вопросов «с кем» и «кто ведёт». НЕ выводи lesson_groups и "
            "teachers в ответ по своей инициативе — только если спросили именно про это."
        ),
        "parameters": {
            "type": "object",
            "properties": {
