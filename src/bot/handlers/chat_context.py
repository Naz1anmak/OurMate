...
def build_time_context_line() -> str:
    ...
    return (
        f"Контекст времени: сейчас {now:%Y-%m-%d %H:%M} ({_WEEKDAY_RU[now.weekday()]}), "
        f"часовой пояс {TIMEZONE}. Сегодня — {now:%Y-%m-%d}. "
        "Относительные даты и время («сегодня», «завтра», «в субботу», «через 10 минут», "
        "«через час», «на следующей неделе») считай от этого момента."
    )
