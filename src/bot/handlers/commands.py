...
async def cmd_start(message: types.Message):
    ...
    _log(f"FP; От {user_login} ({message.from_user.full_name}): /start")
...
• <code>help</code> или <code>команды</code> — Справка по командам
• <code>др</code> — Ближайший день рождения
• <code>др 123456789</code> или <code>др @username</code> — Дата дня рождения пользователя по id или username
• <code>пары</code> — Пары на сегодня
• <code>пары завтра</code> — Пары на завтра
• <code>отписаться</code> — Отключить поздравления (в ЛС с ботом)
<i>❕ Нужно упомянуть бота или ответить на его сообщение.</i>

<b>Команды управления (доступны только владельцу):</b>
• <code>logs</code> — Логи бота
• <code>full logs</code> — Полные логи
• <code>status</code> — Статус службы
• <code>system</code> — Информация о системе
...
Если вам нужны автоматические поздравления с днями рождения в вашей беседе и уведомления о будущих днях рождения, вы можете клонировать этого бот и настроить под себя:
